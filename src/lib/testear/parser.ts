...
export function parseQuestions(text: string, fileName: string): Question[] {
  ...
  console.log(`[Parser] Found ${blocks.length} potential question blocks`);
  ...
  for (const raw of blocks) {
    ...
    if (!m) {
      invalidBlocks++;
      if (invalidBlocks <= 3) {
        console.debug(`[Parser] Block skipped (pattern mismatch): "${block.substring(0, 50)}..."`);
      }
      continue;
    }
  ...
  console.log(`[Parser] Trying fallback line-by-line format`);
  const lines = cleaned
    .split("\n")
    .map((l) => l.trim())
    .filter(Boolean);
  ...
  console.log(`[Parser] Fallback: Parsed ${fallbackQuestions.length} questions (line-by-line)`);
